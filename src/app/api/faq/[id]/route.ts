/**
 * @fileoverview Single FAQ API Route (Dynamic)
 * @description Belirli bir SSS öğesi için ID bazlı CRUD operasyonları.
 ...
 * Tekil SSS düzenleme ve silme işlemleri için kullanılır.
 * Not: Bu route /api/faq/route.ts'deki koleksiyon endpoint'i ile birlikte çalışır.
 * @version 1.0.0
 */
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (body.question !== undefined) {
      ...
      updateData.question = sanitizeInput(body.question);
    }
    ...
    if (body.answer !== undefined) {
      ...
      updateData.answer = sanitizeInput(body.answer);
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'SSS öğesi başarıyla güncellendi',
        faq,
      },
      { headers }
    );
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'SSS öğesi başarıyla silindi',
      },
      { headers }
    );
  } catch (error) {
